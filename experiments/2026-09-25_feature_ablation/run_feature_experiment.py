...
실행 (저장소 루트에서):
    PYTHONHASHSEED=42 KAMP_FAST=0 python -X utf8 experiments/2026-09-25_feature_ablation/run_feature_experiment.py
"""
...
FINAL = "2단계 레짐(3분류)"
EXP_SCORE = pd.read_csv(REPO / "outputs/tables/ch2_scorecard.csv", encoding="utf-8-sig").set_index("모델").loc[FINAL]
EXP_ABL = pd.read_csv(REPO / "outputs/tables/ch2_ablation.csv", encoding="utf-8-sig").set_index("제거 피처군")
...
print(f"[완료] {meta['total_sec']}s — 결과: {OUT.name}/", flush=True)
